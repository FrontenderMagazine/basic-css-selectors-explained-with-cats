...
`.parent`, независимо от того, насколько глубоко они находятся в DOM-дереве. Чтобы получить такой селектор, нужно поместить между двумя селекторами обычный пробел. В этом примере мы получим все дочерние `.child` вложенные в `.parent`:
